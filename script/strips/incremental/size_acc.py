...
#Test for one domain
def test_domain(reference, domain_to_test, domain, rep_plan, it):
    for problem in range(1,21):
        ...
        if(code_return == 0):
            code_return = os.system("cat plan.txt > %s/plan_%d_%d 2> /dev/null " % (rep_plan, problem, it))
            if(code_return == 0):
                continue
            else:
                os.system("echo \"NO SOLUTION\" > %s/plan_%d_%d " % (rep_plan, problem, it))
        else:
            os.system("echo \"NO SOLUTION\" > %s/plan_%d_%d " % (rep_plan, problem, it))
        # return
...
def test(domain, base_dir):
    ...
    for scenar in [1,2,5,10]:
        rep_plan = "%s/%s/Size%d/plans" % (base_dir, domain, scenar)
            ...
            for run in [0,1,2,3,4]:
                ...
                for partial in [100, 25]:
                    ...
                    for noise in [0, 20]:
                        ...
                        os.mkdir(rep_plan4)
                        for it in [5,10,15,20,25,30,35,40,45,50]:
                            domain_to_test=("%s/%s/%s/IS%d/domain.%d.%d.%d.%d.pddl " % (base_dir, domain, scenar, initial, partial, noise, run, it))
                            test_domain(reference, domain_to_test, domain, rep_plan4, it)
                        # return
