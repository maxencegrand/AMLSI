...
def run(domain, domain_name, size, initial_state, rep, tabu=False):
    command = ("%s -t RPNIPC -min %d -max %d -T %d -htn -recomposition heuristic -properties %s -d %s -o %s -n %s -i %s -r %d") % \
                (RUNNABLE,\
                 size[1],\
                 size[1],\
                 size[0],\
...
def test(domain, domain_name, base_dir):
    ...
    for size in [[1,5],[2,5],[3,5],[5,5],[5,10],[10,10],[15,10],[20,15]]:
        learnt_rep1 = "%s/size_%d_%d" % (learnt_rep0, size[0], size[1])
        os.mkdir(learnt_rep1)
        initial_state= "%s/initial_states/initial.hddl" % (domain_rep)
        run(reference, domain_name, size, initial_state, learnt_rep1)
