...
public class HtnDomain extends Domain{
	...
	public Pair<Float, Float> test(
		...
		return new Pair<>(fr.uga.amlsi.utils.Utils.mean(succes), fr.uga.amlsi.utils.Utils.mean(correct));
	}
	...
	public float testNegative(
		...
		return fr.uga.amlsi.utils.Utils.mean(precision);
	}
...
//			System.out.println(op+" "+op.getParameters());
			...
			for(Symbol a : op.allInstances(contants)) {
				Map<String, String> m = fr.uga.amlsi.utils.Utils.reverseParamMap(a.mapping());
