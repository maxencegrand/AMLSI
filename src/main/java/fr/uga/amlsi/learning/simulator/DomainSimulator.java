...
public class DomainSimulator extends StripsSimulator{
	...
	public void makeMovement(Movement movement) {
		...
		for(Symbol a : this.instances.get(movement.getOperator())) {
			...
			prec = prec.instanciate(fr.uga.amlsi.utils.Utils.reverseParamMap(a.mapping()));
			Observation post = postconditions.get(a.generalize());
			post = post.instanciate(fr.uga.amlsi.utils.Utils.reverseParamMap(a.mapping()));
			Symbol p = movement.getPredicate().generalize(fr.uga.amlsi.utils.Utils.reverseParamMap(a.mapping()));
...
//		}
    ...
    public float fscore(Sample pos, Sample neg) {
    	...
    	recall = fr.uga.amlsi.learning.Measure.accuracy(truePos, pos.size());
    	//Compute Precision
    	...
    	precision = fr.uga.amlsi.learning.Measure.P(truePos, falsePos);
    	//Compute fscore
    	fscore = fr.uga.amlsi.learning.Measure.FScore(recall, precision);
    	return fscore;
    }
