	...
	public void backtrackPrecondition(Observation initial, Example ex) {
		...
		for(int i =0; i < ex.size(); i++) {
			...
			Observation prec = this.domain.get(op).getX().instanciate(
					fr.uga.amlsi.utils.Utils.reverseParamMap(act.mapping()));
...
//			}
	...
	(Map<Pair<Symbol, String>, List<Symbol>> m, Observation is, Sample pos) {
		for(Trace t : pos.getExamples()) {
			...
			for(int i = 0; i < ex.size(); i++) {
				...
				eff = eff.instanciate(fr.uga.amlsi.utils.Utils.reverseParamMap(
						act.mapping()));
				if(i < ex.size()-1) {
					Symbol op = act.generalize();
					...
					if(m.containsKey(new Pair<>(op, next))) {
						...
						prec = prec.instanciate(fr.uga.amlsi.utils.Utils.reverseParamMap(
								ex.get(i+1).mapping()));
	...
	public Observation apply(Observation state, Symbol action) {
		...
		eff = eff.instanciate(fr.uga.amlsi.utils.Utils.reverseParamMap(action.mapping()));
	...
	public float fscore(Observation is, Sample pos, Sample neg) {
		...
		fscore = fr.uga.amlsi.learning.Measure.FScore(
				fr.uga.amlsi.learning.Measure.R(truePos, pos.size()), 
				fr.uga.amlsi.learning.Measure.P(truePos, falsePos));
		return fscore;
	}
