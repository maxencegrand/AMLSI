            ...
            for f in (20,40,60,80,100):
                for n in (0,1,5,20):
                    for i in [1,2,3]:
                        ...
                        line.append(n)
                        line.append(i)
                        for m in ["Distance", "FScore", "Accuracy", "IPC", "Time"]:
            ...
            for line in raws:
                ...
                if p:
                    if line == "############################################":
                        ...
                        self.data_domain["FScore"][fluent][noise][initial].append(100*float(line[17:]))
                        continue
                    elif line[:4] == "Time":
                        ...
                        self.data_dfa["Time DFA"][initial].append(float(line[17:]))
                        continue
        ...
        for initial in [1,2,3]:
            for obs in [20,40,60,80,100]:
                for noise in [0,1,5,20]:
                    for run in range(len(self.data_domain["FScore"][obs][noise][initial])):
                        file_ = ("%s/%s/%s/IS%d/plans/RUN%d/OBS%d/NOISE%d" % (self.directory, self.domain, self.method, initial, run, obs, noise))
                        domain_file = ("./benchmarks/temporal/%s/domain.pddl" % self.domain)
                        rep_instances = ("./benchmarks/temporal/%s/instances" % self.domain)
                        # # print(file_)
                        accSolve = accuracy_domain(domain_file, rep_instances, file_, 20)
                        #self.data_domain["Solve"][obs][noise][initial].append(float(self.data_domain["FScore"][obs][noise][initial][run]))
                        #self.data_domain["Accuracy"][obs][noise][initial].append(float(self.data_domain["FScore"][obs][noise][initial][run]))
                        #self.data_domain["IPC"][obs][noise][initial].append(float(self.data_domain["FScore"][obs][noise][initial][run])/5)
